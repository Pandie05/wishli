...
/**
 * This function fetches whatever address it is handed, from the server, which
 * is the shape of a server-side request forgery: without a check, anyone
 * signed in could point it at an address only the server can reach and read
 * the answer back through the preview -- a loopback port, something on a
 * private network, or a cloud metadata endpoint.
 *
 * A product link is always a public host, so refusing everything else costs
 * nothing. Hostnames that are not literal IPs are let through; resolving them
 * here would not help much anyway, since the name could resolve differently
 * by the time fetch() runs.
 */
function isPrivateHost(hostname: string): boolean {
  const host = hostname.toLowerCase().replace(/^\[|\]$/g, '')

  if (host === 'localhost' || host.endsWith('.localhost')) return true
  // kubernetes/docker/cloud-internal style names
